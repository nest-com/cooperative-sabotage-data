...
title('ROC Curves for different \lambda values', 'FontSize', 14, 'FontWeight', 'bold', 'Interpreter', 'tex');
grid on;
xlabel('FPR', 'FontSize', 12, 'Interpreter', 'latex');
ylabel('TPR', 'FontSize', 12, 'Interpreter', 'latex');
ylim([-0.1 1.1])
xlim([-0.002 0.02])
legend('\lambda = 0.25 ', '\lambda = 0.50 ', '\lambda = 0.75', '\lambda = 1.00', '\lambda = 1.50', '\lambda = 2.00','Location','southeast','FontSize',12);

set(gca, 'FontSize', 12, 'FontWeight', 'bold');
box off;
plot_fig.Position = [680 458 645 316];
exportgraphics(gca, 'ConvexCombinedROC6Lambdas.png', 'Resolution', 300);
